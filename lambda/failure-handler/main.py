...
# Pages per chunk -- must match the value used by the PDF splitter Lambda
# (lambda/pdf-splitter-lambda/main.py). Kept here so the page-range shown to
# the user is accurate. Overridable via env var without a code change.
...
REASON_SUMMARIES = {
    ...
    "ADOBE_API": "The Adobe PDF Services API (auto-tagging/extraction) failed.",
...
def lambda_handler(event, context):
    ...
    # Build the failed-chunk / page-range summary from whatever detail exists.
    failed_chunks = []
    categories = []
    for err in station_errors:
        ...
        if chunk_index is not None:
            ...
            # Prefer page range from the detail file; otherwise compute it.
            entry["page_start"] = err.get(
                "page_start", (chunk_index - 1) * PAGES_PER_CHUNK + 1
            )
            entry["page_end"] = err.get("page_end", chunk_index * PAGES_PER_CHUNK)
    ...
    print(
        ...
        + (f" | {pages_desc}" if pages_desc else "")
    )
